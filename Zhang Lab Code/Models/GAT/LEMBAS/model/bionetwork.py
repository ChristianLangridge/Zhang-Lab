...
class ProjectInput(nn.Module):
    ...
    def forward(self, X_in: torch.Tensor):
      ...
      #  print('ProjectInput_Xfull', X_full.shape)
        return X_full
...
class GATBioNet(nn.Module):
    """使用 GAT 替代原始的 BioNet 计算信号传播"""
    
    def __init__(self, seed, bionet_params, edge_list, edge_MOA, n_network_nodes, in_dim, hidden_dim, out_dim, heads=4, dropout=0.1, activation='relu', device='cpu', activation_function: str = 'MML', ):
        ...
        # 确保 edge_list 形状正确
        edge_list = np.squeeze(edge_list)  


        # 转换为 PyTorch 张量
        edge_index = torch.tensor(edge_list, dtype=torch.long, device=device)

        # 过滤掉超出 n_network_nodes 的无效索引
        valid_mask = (edge_index[0] < self.n_network_nodes) & (edge_index[1] < self.n_network_nodes)
        edge_index = edge_index[:, valid_mask]  # ✅ 这里修正了 .all(dim=0) 的错误
        ...
        edge_MOA = torch.tensor(edge_MOA, dtype=torch.int, device=device)
      #  edge_MOA = edge_MOA[:, valid_mask]     # 只保留合法边
        self.edge_MOA = edge_MOA    
        # 再创建 mask_MOA，用来标记哪些边是未知（不做符号检查）
        self.mask_MOA = (self.edge_MOA == 0)  # shape 同 edge_MOA，True=未知

        # GAT 结构

        self.gat1 = pyg_nn.GATConv(1, hidden_dim, heads=heads, dropout=dropout).to(device)
        self.gat2 = pyg_nn.GATConv(hidden_dim * heads, 1, heads=1, dropout=dropout).to(device)
        ...
        self.edge_mask = torch.ones(edge_index.shape[1], dtype=torch.bool, device=device)
        num_valid_edges = self.edge_index.shape[1]
        self.edge_weight = nn.Parameter(torch.zeros(num_valid_edges, dtype=torch.float, device=device))
        nn.init.uniform_(self.edge_weight, a=-0.1, b=0.1)  # 随机在 (-0.1, 0.1)
        pos_mask = (self.edge_MOA == 1)
        neg_mask = (self.edge_MOA == -1)

        
        
        signed_moa = edge_MOA[0, :].long() - edge_MOA[1, :].long()
        # signed_moa 形状 [num_edges]，取值 ∈ {+1,0,-1}
        # 存入 self.signed_moa
        self.signed_moa = signed_moa
        # 3) 生成 pos_mask/neg_mask
        pos_mask = (signed_moa == 1)    # 激活边
        neg_mask = (signed_moa == -1)   # 抑制边
        # (未知边就是 0，不归入 pos/neg)
        
        
        self.edge_weight.data[pos_mask] = torch.rand(pos_mask.sum(), device=device) * 0.1
        self.edge_weight.data[neg_mask] = - torch.rand(neg_mask.sum(), device=device) * 0.1
        
    def forward(self, X):
        """使用 GAT 进行前向传播"""
        batch_size = X.shape[0]  # 获取 batch_size
        outputs = []  # 用于存储每个样本的 GAT 计算结果
        e_weight = self.edge_weight.unsqueeze(-1)  # 获取边权重
        for i in range(batch_size):
            x_i = X[i]  # 获取单个样本 (node_num,)
            x_i = x_i.unsqueeze(-1)  # 调整形状为 (node_num, 1)，以适应 GAT
            
            x_i = self.activation(self.gat1(x_i, self.edge_index))
            x_i = self.gat2(x_i, self.edge_index)
            outputs.append(x_i)  # 存储结果
       # print(torch.stack(outputs).shape)
        print('torch.stack(outputs).squeeze(-1)', torch.stack(outputs).squeeze(-1))
        return torch.stack(outputs).squeeze(-1)  # 重新拼接成 (batch_size, node_num, features)
    
    def get_sign_mistmatch(self):
        """
        计算符号不匹配情况：
        - 如果原本是激活边（+1），但 self.edge_weight 变成负值，则 mismatch=1
        - 如果原本是抑制边（-1），但 self.edge_weight 变成正值，则 mismatch=1
        - 如果是未知边（0），则跳过，不计算 mismatch
        """
        learned_sign = torch.sign(self.edge_weight)  # 获取当前 GAT 学到的边权重符号
        
        # 计算 mismatch
        mismatch = (learned_sign != self.signed_moa).float()

        # 忽略未知 MOA 的边 (signed_moa = 0)
        mismatch = mismatch.masked_fill(self.signed_moa == 0, 0.0)
...
class SignalingModel(torch.nn.Module):
    ...
    def __init__(self, net: pd.DataFrame, X_in: pd.DataFrame, y_out: pd.DataFrame,
                ...
                bionet_params: Dict[str, float] = None , 
                 activation_function: str='MML', dtype: torch.dtype=torch.float32, device: str = 'cpu', seed: int = 888, hidden_dim=64, heads=4,                            activation='relu'):
     ...
     #                                   dtype = self.dtype, device = self.device, seed = self.seed)
        ...
        self.signaling_network = GATBioNet( seed =  self.seed,
                                           edge_MOA = edge_MOA,
                                            bionet_params = bionet_params, 
                                            edge_list=edge_list,
                                            n_network_nodes=len(node_labels),
                                            in_dim=len(node_labels),
                                            hidden_dim=hidden_dim,
                                            out_dim=len(node_labels),
                                            heads=heads,
    ...
    def parse_network(self, net: pd.DataFrame, ban_list: List[str] = None, 
                 weight_label: str = 'Interaction', source_label: str = 'TF', target_label: str = 'Gene'):
    ...
    def forward(self, X_in):
     ...
     #   print('****'*10)
    #    print('mode123', X_in.shape)
        X_full = self.input_layer(X_in) # input ligands to signaling network
     #   print('mode123', X_full.shape)
        Y_full = self.signaling_network(X_full) # RNN of full signaling network
     #   print('mode123', Y_full.shape)
